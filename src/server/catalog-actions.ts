...
export type ProductEdit = {
  ...
  mercadoLivreUrl: string; // link do produto no Mercado Livre (opcional)
  active: boolean;
  bestSeller: boolean;
};
  ...
  try {
    ...
    await prisma.product.update({
      ...
      data: {
        ...
        // Só aceita link http(s) do Mercado Livre; vazio limpa o campo.
        mercadoLivreUrl: /^https?:\/\//i.test(data.mercadoLivreUrl.trim())
          ? data.mercadoLivreUrl.trim()
          : null,
        active: data.active,
...
/* ------------------------- AJUSTE DE MARGEM EM MASSA ------------------------- */

/**
 * Recalcula o preço das variações dos produtos selecionados a partir do custo.
 *   - modo "percent": preço = custo × (1 + valor%) + taxaFixa
 *       Ex.: custo 50, 10%, taxa 10  ->  50×1,10 + 10 = 65,00
 *   - modo "fixed":   preço = custo + valor(R$) + taxaFixa
 *       Ex.: custo 50, R$ 15, taxa 10  ->  50 + 15 + 10 = 75,00
 *
 * Aplica SOMENTE a produtos "Nosso" (sem preço sugerido da Laquila) — os
 * "Sugerido" são ignorados para não violar a política da Laquila.
 * Variações sem custo (custo <= 0) são puladas (não dá pra calcular).
 */
export async function bulkApplyMargin(
  ids: string[],
  opts: { mode: "percent" | "fixed"; value: number; fee: number },
): Promise<ActionResult> {
  await requireAdmin();
  if (!ids.length) return { ok: false, message: "Nenhum produto selecionado." };
  const mode = opts.mode === "fixed" ? "fixed" : "percent";
  const value = Number(opts.value);
  const f = Number(opts.fee);
  if (!Number.isFinite(value) || value < 0) return { ok: false, message: "Lucro/margem inválido." };
  if (!Number.isFinite(f) || f < 0) return { ok: false, message: "Taxa inválida." };
  ...
  try {
    ...
    let produtos = 0;
    let variacoes = 0;
    let ignoradosSugerido = 0;
    let ignoradosSemCusto = 0;

    for (const p of products) {
      // "Nosso" = a variação mais barata NÃO tem preço sugerido da Laquila.
      ...
      if (cheapest?.valorSugerido && cheapest.valorSugerido > 0) {
        ignoradosSugerido++;
        continue;
      }
      ...
      for (const v of p.variants) {
        ...
        const base = mode === "fixed" ? custo + value : custo * (1 + value / 100);
        const novoPreco = Math.round((base + f) * 100) / 100;
        if (novoPreco !== v.price) {
          await prisma.variant.update({ where: { id: v.id }, data: { price: novoPreco } });
          variacoes++;
          alterou = true;
        }
      }
    ...
    revalidatePath("/");
    const msg =
      `Margem aplicada: ${produtos} produto(s), ${variacoes} variação(ões).` +
      (ignoradosSugerido ? ` ${ignoradosSugerido} com preço sugerido (Laquila) ignorado(s).` : "") +
      (ignoradosSemCusto ? ` ${ignoradosSemCusto} variação(ões) sem custo ignorada(s).` : "");
    return { ok: true, message: msg };
  } catch (err) {
    console.warn("[catalog] bulkApplyMargin:", (err as Error)?.message);
    return { ok: false, message: "Erro ao aplicar a margem." };
  }
}
...
export type ManualProductInput = {
  ...
  cost: number;
  stock: number;
  sku: string; // SKU real (opcional) — p/ produto único, sincroniza com a Laquila
  sizes: string; // "P, M, G" OU "P=1700012, M=1700013" (tamanho=SKU) — opcional
  description: string;
  image: string;
  weight: number; // kg
  length: number; // cm
...
export async function createProductManual(input: ManualProductInput): Promise<ActionResult> {
  ...
  try {
    ...
    // Cada tamanho pode vir como "P" ou "P=1700012" (tamanho=SKU real da Laquila).
    const parsed = input.sizes
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
      .map((entry) => {
        ...
        return eq >= 0
          ? { size: entry.slice(0, eq).trim() || null, sku: entry.slice(eq + 1).trim() }
          : { size: entry, sku: "" };
      });
    // Sem tamanhos -> 1 variação (usa o SKU único, se informado).
    const list = parsed.length ? parsed : [{ size: null as string | null, sku }];

    // Validações de SKU: sem repetição e sem colidir com o catálogo existente.
    const provided = list.map((s) => s.sku).filter(Boolean);
    if (new Set(provided).size !== provided.length) {
      return { ok: false, message: "Há SKUs repetidos entre os tamanhos." };
    }
    if (provided.length) {
      const existing = await prisma.variant.findMany({ where: { cdItem: { in: provided } }, select: { cdItem: true } });
      if (existing.length) {
        return {
          ok: false,
          message: `SKU já existe no catálogo: ${existing.map((e) => e.cdItem).join(", ")}. Use "Importar" para atualizar em vez de recriar.`,
        };
      }
    }

    const variants = list.map((s, i) => ({
      cdItem: s.sku || `MAN-${hash}-${i}`,
      size: s.size,
      price: input.price,
