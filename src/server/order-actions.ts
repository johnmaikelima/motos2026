...
import { sendOrderPlacedCustomer, sendNewOrderAdmin, type OrderEmailData } from "@/lib/email";
...
export type CheckoutInput = {
  ...
  giftSize?: string; // tamanho escolhido p/ o brinde (validado no servidor)
  shipping?: { price: number; method: string; carrier?: string; service?: string };
};
...
export async function submitOrder(input: CheckoutInput): Promise<CheckoutResult> {
  ...
  // BRINDE (1 por pedido) — recalculado no servidor; nunca confia no cliente.
  // Adiciona como item de R$ 0. O tamanho vem do cliente, mas é validado contra
  // as variações COM ESTOQUE; se inválido, usa a primeira disponível.
  try {
    const gift = await resolveCartGift(productIds);
    if (gift) {
      const chosen =
        gift.variants.find((v) => (v.size ?? "") === (input.giftSize ?? "")) ?? gift.variants[0];
      if (chosen) {
        orderItems.push({
          cdItem: chosen.cdItem,
          productName: `${gift.name} (Brinde)`,
          size: chosen.size,
          qty: 1,
          price: 0,
        });
    ...
    console.warn("[checkout] brinde não aplicado:", (e as Error)?.message);
  }
  ...
  try {
    ...
    // transportadora/serviço (carrier/service do Envia) — necessários p/ gerar etiqueta.
    let shippingCarrier = input.shipping?.carrier ?? null;
    let shippingService = input.shipping?.service ?? null;
    try {
      ...
      if (ship.ok && ship.options.length > 0) {
        const match = shippingMethod
          ...
          : undefined;
        const chosen = match ?? ship.options.reduce((a, b) => (a.price <= b.price ? a : b));
        // usa o preço REAL da cotação (a opção escolhida, ou a mais barata)
        shippingPrice = chosen.price;
        shippingCarrier = chosen.company || shippingCarrier;
        shippingService = chosen.service || shippingService;
      }
    ...
    // Notificações por e-mail (não bloqueiam o pedido se falharem).
    const emailData: OrderEmailData = {
      number,
      customerName: input.nome,
      email,
      items: orderItems.map((i) => ({ name: i.productName, size: i.size, qty: i.qty, price: i.price })),
      subtotal: total,
      shipping: shippingPrice,
      total: total + shippingPrice,
      discount: 0,
      address: { street: input.endereco, number: input.numero, city: input.cidade, uf: input.uf, cep },
    };
    await sendOrderPlacedCustomer(emailData);
    await sendNewOrderAdmin(emailData);
