...
/** Envia UM modelo específico (dados de exemplo) para o e-mail informado. */
export async function sendModelSampleAction(kind: EmailKind, to: string): Promise<TestEmailResult> {
  await requireAdmin();
  const dest = (to || "").trim();
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(dest)) {
    return { ok: false, message: "Informe um e-mail de destino válido." };
  ...
  try {
    await sendModelSample(kind, dest);
    return { ok: true, message: `Modelo enviado para ${dest}. Confira a caixa (e o spam).` };
  } catch (err) {
    return { ok: false, message: (err as Error)?.message ?? "Falha ao enviar o modelo." };
  }
}
