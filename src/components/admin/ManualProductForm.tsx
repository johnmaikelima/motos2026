  ...
  return (
    <div className="card p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label>
          <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-400">SKU (produto único)</span>
          <input value={f.sku} onChange={(e) => set("sku", e.target.value)} placeholder="SKU real da Laquila (opcional)" className={inputCls} />
        </label>
        <label className="sm:col-span-2">
          <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-400">Tamanhos (vírgula) — ou tamanho=SKU</span>
          <input value={f.sizes} onChange={(e) => set("sizes", e.target.value)} placeholder="P, M, G, GG   ·   ou com SKU: P=1700012, M=1700013" className={inputCls} />
          <span className="mt-1 block text-[11px] text-gray-500">
            Informe o SKU real da Laquila (aqui, ou no campo acima p/ produto único) para o <strong>estoque sincronizar automaticamente</strong>. Sem SKU, geramos um interno (não sincroniza).
          </span>
        </label>
        <label>
