...
import { uploadLogo } from "@/server/upload-actions";
...
export default function StoreSettingsForm({ initial }: { initial: StoreSettings }) {
  ...
  async function onLogo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    else if (!r.ok) alert(r.message ?? "Erro ao enviar o logo.");
  }
            ...
            <div className="flex items-center gap-3">
              {s.logoUrl ? (
                <div className="relative h-12 w-32 overflow-hidden rounded bg-ink-700">
                  <Image src={s.logoUrl} alt="logo" fill className="object-contain" sizes="128px" unoptimized={s.logoUrl.startsWith("data:")} />
                </div>
              ) : (
