...
export default async function ProductPage({
  ...
  const [category, gift, reviews, reviewState, sizeChart, related, settings, SITE_URL] = await Promise.all([
    ...
    <div className="container-rm py-10">
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Alternativa de compra pelo Mercado Livre (prova social p/ quem ainda
              não conhece a loja). Só aparece se o link estiver cadastrado. */}
          {product.mercadoLivreUrl && (
            <a
              href={product.mercadoLivreUrl}
              target="_blank"
              rel="noopener noreferrer nofollow"
              className="mt-3 flex items-center justify-center gap-2 rounded-lg border border-[#ffe600]/50 bg-[#ffe600]/10 px-4 py-3 text-center text-sm font-semibold text-[#ffe600] transition hover:bg-[#ffe600]/20"
            >
              <span aria-hidden>🛡️</span>
              Prefere? Compre com a garantia do <strong>Mercado Livre</strong> ↗
            </a>
          )}
