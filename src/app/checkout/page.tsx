...
import { useCartGift } from "@/lib/use-cart-gift";
import GiftCartLine from "@/components/GiftCartLine";
...
export default function CheckoutPage() {
  ...
  const { gift, size: giftSize, setSize: setGiftSize } = useCartGift(items.map((i) => i.productId));
  ...
  type Frete = { id: number; name: string; company: string; service: string; price: number; deliveryTime: number };
  ...
  // Recalcula o frete sempre que houver CEP válido e itens — cobre o
  // preenchimento automático do login (sem depender de clicar no campo) e o
  // tempo de hidratação do carrinho. Evita recalcular o mesmo CEP.
  const lastFreteCep = useRef("");
  useEffect(() => {
    const cep = (values.cep ?? "").replace(/\D/g, "");
    if (cep.length === 8 && items.length > 0 && lastFreteCep.current !== cep) {
      lastFreteCep.current = cep;
      calcFrete(cep);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [values.cep, items.length]);

  function setVal(name: string, v: string) {
    setValues((prev) => ({ ...prev, [name]: v }));
  }

  // Dispara o evento de compra (GA4 / Google Ads) uma única vez.
  ...
  function trackPurchase(value: number, transactionId: string) {
    ...
    gaPurchase({
      transactionId,
      value,
      shipping: freteSel?.price ?? 0,
      items: items.map((i) => ({ item_id: i.productId, item_name: i.name, quantity: i.qty, price: i.price })),
    });
  }
  ...
  useEffect(() => {
    ...
    if (paymentId && payerId) {
      ...
      executePlusPaymentAction(paymentId, payerId).then((res) => {
        if (res.ok) {
          trackPurchase(subtotal + (freteSel?.price ?? 0), order?.numero ?? paymentId);
          clear();
          setStep("done");
        } else {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await submitOrder({
      ...
      giftSize: gift ? giftSize : undefined,
      shipping: freteSel
        ? { price: freteSel.price, method: `${freteSel.company} ${freteSel.name}`, carrier: freteSel.company, service: freteSel.service }
        : undefined,
    });
    setLoading(false);
    if (res.ok) {
      ...
      <div className="grid gap-8 lg:grid-cols-[2fr_1fr]">
        {step === "form" ? (
          ...
          <form onSubmit={handleSubmit} className="card grid grid-cols-2 gap-4 p-6">
              ...
              <div key={f.name} className={f.half ? "col-span-1" : "col-span-2"}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Frete é recalculado pelo efeito (observa o CEP). Aqui só buscamos o endereço.
                    if (f.name === "cep" && e.target.value.replace(/\D/g, "").length === 8) {
                      lookupCep(e.target.value);
                    }
                  }}
                  onBlur={f.name === "cep" ? (e) => lookupCep(e.target.value) : undefined}
            ...
            <button
              type="submit"
              disabled={loading || freteLoading || (fretes.length > 0 && !freteSel)}
              className="btn-primary col-span-2 mt-2 disabled:opacity-50"
            >
              {loading ? "Salvando pedido..." : freteLoading ? "Calculando frete..." : "Ir para o pagamento"}
            </button>
          </form>
          </div>
        ) : (
          <div className="card p-6">
            ...
            {method === "card" && order && payer && (
              <PaypalPlus
                ...
                payer={payer}
                onPaid={() => {
                  trackPurchase(subtotal + (freteSel?.price ?? 0), order.numero);
                  clear();
                  setStep("done");
                }}
              />
            ...
            {method === "pix" && order && (
              <PixPayment
                localOrderId={order.id}
                onPaid={() => {
                  trackPurchase(pixPrice(subtotal, pixPct) + (freteSel?.price ?? 0), order.numero);
                  clear();
                  setStep("done");
                }}
              />
        ...
        <aside className="card h-max p-6">
          ...
          <ul className="mt-4 flex flex-col gap-3">
            ...
            {gift && <GiftCartLine gift={gift} size={giftSize} setSize={setGiftSize} compact />}
          </ul>
