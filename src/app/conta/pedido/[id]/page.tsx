...
export default async function MeuPedidoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Link de rastreio por transportadora (quando houver código).
  function trackingUrl(carrier: string | null, code: string): string | null {
    const c = (carrier ?? "").toLowerCase();
    if (c.includes("correios")) return `https://www.linkcorreios.com.br/?id=${encodeURIComponent(code)}`;
    if (c.includes("jadlog")) return "https://www.jadlog.com.br/tracking";
    return null;
  }
  const rastreioLink = order.trackingCode ? trackingUrl(order.shippingCarrier, order.trackingCode) : null;
          ...
          <dl className="mt-3 space-y-2 text-sm">
            ...
            {order.shippingMethod && (
              <div className="flex justify-between gap-4">
                ...
                <dd className="text-right text-white">{order.shippingMethod}</dd>
              </div>
            )}
          </dl>

          {order.trackingCode && (
            <div className="mt-4 rounded-lg border border-lime/30 bg-lime/5 p-4 text-center">
              <p className="text-xs font-bold uppercase tracking-wide text-gray-400">Código de rastreamento</p>
              <p className="mt-1 select-all text-lg font-extrabold tracking-wide text-lime">{order.trackingCode}</p>
              {rastreioLink && (
                <a
                  href={rastreioLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-2 inline-block text-xs font-semibold text-white underline underline-offset-2 hover:text-lime"
                >
                  Rastrear encomenda ↗
                </a>
              )}
