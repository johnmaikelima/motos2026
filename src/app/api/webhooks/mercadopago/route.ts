...
import { sendOrderPaidCustomer } from "@/lib/email";
  ...
  try {
    ...
    if (paymentId && (evt === "" || evt.includes("payment"))) {
      ...
      if (pay?.status === "approved") {
        const order = await prisma.order.findFirst({ where: { paymentId: String(paymentId) }, include: { items: true } });
        if (order && !order.paidAt) {
          await prisma.order.update({ where: { id: order.id }, data: { status: "pago", paidAt: new Date() } });
          revalidatePath("/admin/pedidos");
          await sendOrderPaidCustomer(toOrderEmailData(order));
        }
      }
    }
  } catch (err) {
